...
export class BaseCrudService {
  ...
  static async create<T extends WixDataItem>(
    ...
    try {
      ...
      // Fallback на localStorage если Wix API недоступен (dev режим)
      const errorMsg = error instanceof Error ? error.message : String(error);
      if (errorMsg.includes('MetaSite') || errorMsg.includes('WDE0117')) {
        console.warn(`Wix API not available for ${collectionId}, using localStorage instead`, errorMsg);
        
        try {
          // Динамически импортируем MockDataService
          const { MockDataService: MockService } = await import('../mock-data-service');
          const item = { ...itemData } as any;
          if (!item._id) {
            item._id = `mock-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
          }
          const result = await MockService.create(collectionId, item);
          console.log(`Created item in mock storage for ${collectionId}:`, result._id);
          return result as T;
        } catch (mockError) {
          console.error(`Failed to fallback to mock storage:`, mockError);
          throw mockError;
        }
      }
    ...
    try {
      ...
      if (errorMsg.includes('MetaSite') || errorMsg.includes('WDE0117')) {
        ...
        try {
          ...
          const mockResult = await MockService.getAll<T>(
            collectionId,
            undefined,
            { limit, offset: skip }
          );
          
          const hasNext = skip + limit < mockResult.totalCount;
          
          console.log(`Loaded from mock storage for ${collectionId}: ${mockResult.items.length} items`);
          ...
          console.error(`Failed to fallback to mock storage for ${collectionId}:`, mockError);
          throw mockError;
        }
      }
